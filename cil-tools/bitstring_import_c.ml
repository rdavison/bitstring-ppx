...
OPTIONS" in
  ...
  let tmp, delete_tmp =
    if not save_temps then (
      let tmp = Filename.temp_file "" ".i" in
  ...
  let cmd =
    sprintf "cpp %s -I %s -include bitstring-import-prefix.h %s > %s"
      ...
      (Filename.quote (Findlib.default_location () // "bitstring"))
  ...
  List.iter (
    fun (tname, ttype, loc) ->
      ...
      and pattern_field_of_int fname bitsoffset bitswidth ikind endian =
        let signed = isSigned ikind in
        let _loc = loc_of_cil_loc loc in
      ...
      (* Convert a CIL location into an OCaml location.  Grrr these
       * should be compatible!
       *)
      and loc_of_cil_loc loc =
        let _loc =
          {Lexing.pos_fname = loc.file;
           pos_lnum = loc.line;
           pos_bol = 0;
           pos_cnum = loc.byte}
        in
        {Location.loc_start = _loc; loc_end = _loc; loc_ghost = false}
      in
