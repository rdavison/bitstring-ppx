...
and libpcap_header bits =
  ...
  | ((0xa1b2c3d4_l|0xd4c3b2a1_l) as magic) [@l 32], (* magic number *)
    major [@l 16] [@endian (endian_of magic)],     (* version *)
    minor [@l 16] [@endian (endian_of magic)],
    timezone [@l 32] [@endian (endian_of magic)],  (* timezone correction (secs)*)
    _ [@l 32] [@endian (endian_of magic)], (* always 0 apparently *)
    snaplen [@l 32] [@endian (endian_of magic)],  (* max length of capt pckts *)
    network [@l 32] [@endian (endian_of magic)],  (* data link layer type *)
    rest [@l -1] [@bitstring]
    ->
...
and libpcap_packet e file_header bits =
  match%bitstring bits with
  |  ts_sec [@l 32] [@endian e],        (* packet timestamp seconds *)
     ts_usec [@l 32] [@endian e],  (* packet timestamp microseconds *)
     incl_len [@l 32] [@endian e], (* packet length saved in this file *)
     orig_len [@l 32] [@endian e], (* packet length originally on wire *)
     pkt_data [@l Int32.to_int incl_len*8] [@bitstring],
     rest [@l -1] [@bitstring]
    ->
...
and decode_and_print_packet file_header pkt_header pkt_data =
  ...
  *)
  (match%bitstring pkt_data with
   | d0 [@l 8], d1 [@l 8], d2 [@l 8], d3 [@l 8], d4 [@l 8], d5 [@l 8], (* ether dest *)
     s0 [@l 8], s1 [@l 8], s2 [@l 8], s3 [@l 8], s4 [@l 8], s5 [@l 8], (* ether src *)
     _ [@l 16],                                        (* ethertype *)
     packet [@l -1] [@bitstring]                        (* payload *)
     ->
       ...
       (match%bitstring packet with
        | 4 [@l 4],                     (* IPv4 *)
          hdrlen [@l 4], tos [@l 8], length [@l 16],
          identification [@l 16], flags [@l 3], fragoffset [@l 13],
          ttl [@l 8], protocol [@l 8], checksum [@l 16],
          s0 [@l 8], s1 [@l 8], s2 [@l 8], s3 [@l 8],
          d0 [@l 8], d1 [@l 8], d2 [@l 8], d3 [@l 8],
          _(*options*) [@l (hdrlen-5)*32] [@bitstring],
          _(*payload*) [@l -1] [@bitstring] ->
            printf "IPv4 %d.%d.%d.%d < %d.%d.%d.%d "
              s0 s1 s2 s3 d0 d1 d2 d3

        | 6 [@l 4],                     (* IPv6 *)
          tclass [@l 8], flow [@l 20],
          length [@l 16], nexthdr [@l 8], ttl [@l 8],
          _(*source*) [@l 128] [@bitstring],
          _(*dest*) [@l 128] [@bitstring],
          _(*payload*) [@l -1] [@bitstring] ->
            printf "IPv6 ";
